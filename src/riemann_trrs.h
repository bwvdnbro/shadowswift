 ...
 */
__attribute__ ((always_inline)) INLINE static void riemann_solver_solve(GFLOAT* WL, GFLOAT* WR, GFLOAT* Whalf, float* n_unit){
    GFLOAT aL, aR;
    GFLOAT PLR;
    GFLOAT vL, vR;
    GFLOAT ustar, pstar;
    ...
    ustar = ( PLR * vL / aL + vR / aR + const_riemann_tdgm1 * ( PLR - 1.0f ) ) / ( PLR / aL + 1.0f / aR );
    pstar = 0.5f * ( WL[4] * pow( 1.0f + const_riemann_gm1d2 / aL * ( vL - ustar ), const_riemann_tgdgm1 )
                     + WR[4] * pow( 1.0f + const_riemann_gm1d2 / aR * ( ustar - vR ), const_riemann_tgdgm1) );
    ...
    if( ustar < 0.0f ){
        ...
        if( SHR > 0.0f ){
            ...
            if( STR <= 0.0f ){
                Whalf[0] = WR[0] * pow( const_riemann_tdgp1 - const_riemann_gm1dgp1 / aR * vR, const_riemann_tdgm1 );
                vhalf = const_riemann_tdgp1 * ( -aR + const_riemann_gm1d2 * vR ) - vR;
                Whalf[4] = WR[4] * pow( const_riemann_tdgp1 - const_riemann_gm1dgp1 / aR * vR, const_riemann_tgdgm1 );
            } else {
        ...
        if( SHL < 0.0f ){
            ...
            if( STL > 0.0f ){
                Whalf[0] = WL[0] * pow( const_riemann_tdgp1 + const_riemann_gm1dgp1 / aL * vL, const_riemann_tdgm1 );
                vhalf = const_riemann_tdgp1 * ( aL + const_riemann_gm1d2 * vL ) - vL;
                Whalf[4] = WL[4] * pow( const_riemann_tdgp1 + const_riemann_gm1dgp1 / aL * vL, const_riemann_tgdgm1 );
            } else {
